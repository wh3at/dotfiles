---
name: clear-japanese
description: ユーザーが日本語で書きかけてきたら、応答を書く前にこのスキルを必ず参照する。会話、説明、提案、相談、コードコメント、コミットメッセージ、ドキュメント生成まで、日本語で書くあらゆる場面で適用する。明瞭・簡潔・LLM 口調のない日本語に整えるための規範。ユーザーの入力が日本語なら、タスクの種類（コーディング、デバッグ、設計相談、雑談）を問わずトリガーする。日本語で返事をする気配があれば、ためらわずにこのスキルを読むこと。
---
